...
std::map<std::string, std::weak_ptr<sf::Texture>> ControladorAnimacion::m_textures;

ControladorAnimacion::ControladorAnimacion(std::string const& id)
    : m_texture(load(id)){}

ControladorAnimacion::~ControladorAnimacion(){}

sf::Texture* ControladorAnimacion::get()
{
    if (!m_texture){ return nullptr;  }
    return m_texture.get();
}

std::shared_ptr<sf::Texture> ControladorAnimacion::load(std::string const& id)
{
